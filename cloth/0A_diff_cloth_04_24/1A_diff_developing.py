...
import os
import time
from geometries import Obj, Hanger, Path, Cloth
from math import pi
...
def Grid_To_Particle(t: ti.i32, obj:ti.template()):
    ...
    for p in range(obj.pio_n):
        obj.pio_x[t + 1,p] = obj.x[int(p*obj.pio_scale)]
        obj.pio_x_t[p] = obj.x[t + 1,int(p*obj.pio_scale)]
        obj.pio_v[t + 1,p] = obj.v[t + 1,int(p*obj.pio_scale)]
...
def ini_v_input():
    for p in range(v_input_num):
        v_input[p] = ti.Vector([0.0, 0.0,0.5])
...
@ti.kernel
def compute_loss(obj:ti.template(), v_input: ti.template(), minimum_index: ti.i32):
    for p in range(obj.pio_n):
        dist = ti.math.length(obj.pio_x[minimum_index, p] - ti.Vector([0.5,0.5,0.7]))
        loss[None] += dist 
...
def main():
    ...
    #obj.initialize()
    cloth_0.initialize(cloth_0.x_np,cloth_0.normal_np,cloth_0.mesh_np)
