...
def set_render():
    ...
    camera = ti.ui.Camera()
    scale = 1.2
    camera.position(0.9*scale, 0.9*scale, 0.6*scale)
    camera.lookat(0, 0, 0)
    camera.up(0, 0, 1)
...
def render(obj_list, manipulator_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,ball_center, ball_radius):
    ...
    for i in range(len(obj_list)):
        #scene.particles(obj_list[i].x, radius = obj_list[i].e_radius * 0.3 , color = (0,0,0))
        scene.mesh(obj_list[i].x, indices = obj_list[i].mesh_indices, color = obj_list[i].color)
        #scene.lines(obj_list[i].normal_line, 3, obj_list[i].normal_line_indices, (0.0, 0.0, 0.0))
