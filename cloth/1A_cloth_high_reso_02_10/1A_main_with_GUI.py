...
import taichi as ti
from geometries import Obj, Cloth, Cylinder
...
E, eta = 4e2, 0.3 # Young's modulus, Poisson's ratio
...
# e_radius, pho = 0.001, 2.0 # e_radius is the radius of the element. The distance between two elements is designed as 2 * e_radius.
# color = (0.5,0.5,0.8)
# obj_0 = Obj(center, dimension, axis_angle, e_radius, pho, color)

# Cloth
name = 'TShirt'
center = ti.Vector([0.5, 0.5, 0.2 ]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
center, quaternion, radius, height, friciton, resolution, e_radius = ti.Vector([0.45, 0.5, 0.3]), ti.Vector([0.0, 1.0, 0.0, 0.0]), 0.02, 0.02, 0.3, 32, 0.001 # friction and speed are not used yet
...
def Particle_To_Grid(obj:ti.template()):
    ...
    for p in obj.x:
        ...
        # # Air force
        # normal = F_p @ ti.Vector([0.0, 0.0, 1.0])
        # v_n = normal.dot(v_p) * normal # normal velocity
        # v_l = v_p - v_n             # parallel velocity
        # if p == 0:
        #     print(ti.math.length(v_p) - (ti.math.length(v_l) ** 2 + ti.math.length(v_n) ** 2) ** 0.5)
        #factor_n = (1 - ti.exp(-1 * dt)) * 10
        #factor_l = (1 - ti.exp(-0.5 * dt)) * 2
        #factor_n = 0.002
        #factor_l = 0.001
        
        # compute air drag
        # normal = ti.math.normalize(F_elastic @ ti.Vector([0.0, 0.0, 1.0]))
        # projection = ti.math.dot(v_p, normal)
        # drag_factor = 1e-2
        # v_p -= drag_factor * projection
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            #grid_v[base + offset] -= weight * m * factor_n * v_n
            #grid_v[base + offset] -= weight * m * factor_l * v_l
...
def quaternion_to_axis(arg):
    theta = 2 * ti.math.acos(arg[0])
    x = arg[1] / (ti.math.sin(theta/2) + 1e-10)
    y = arg[2] / (ti.math.sin(theta/2) + 1e-10)
    z = arg[3] / (ti.math.sin(theta/2) + 1e-10)
    axis_angle = ti.Vector([theta, x, y, z])
    return axis_angle
...
# A = [0.0, 1.0, 0.0, 0.0]
# quat = axis_to_quaternion(A)
# axis_A = quaternion_to_axis(quat)
#print(axis_A)

@ti.kernel
def rotate(quat:ti.template()):
    cloth_0.rotate(quat)
...
def main():
    ...
    while window.running:
        ...
        # if counter % 25 == 0:
        #     x_np = obj.x.to_numpy()
        #     np.save(f'./obj/{counter}.npy',x_np)
        #     print(counter)
        #window.show()
        counter += 1
