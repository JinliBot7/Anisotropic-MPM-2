...
import os
import time
from geometries import Hanger, Path, Cloth
from math import pi
...
#ti.init(debug=True)
ti.init(ti.cuda,device_memory_fraction = 0.9,debug=True)
#ti.init(ti.cpu)
real = ti.f32
...
max_MPC_iter = 20 # maximum number of MPC update
max_step = 1000 # step per iteration
max_iter = 2 # max iteration in a MPC update
...
loss_field = scalar()
ti.root.place(loss, loss_standard,loss_position,loss_velocity)
...
# color = (0.5,0.5,0.8)
# cloth_0 = cloth_0(center, dimension, axis_angle, e_radius, total_mass, pho, color, max_step)


# Cloth
name = 'TShirt'
center = ti.Vector([0.5, 0.5, 0.35 ]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
color = (0.5,0.5,0.8)
cloth_0 = Cloth(name, center, axis_angle, e_radius, pho, color, max_step)

dim = 3
neighbour = (3,) * dim

# pio_target = vec()
# ti.root.dense(ti.l, cloth_0.pio_n).place(pio_target)

@ti.kernel
def Particle_To_Grid(t: ti.i32, cloth_0:ti.template()):
    for p in range(cloth_0.n):
        x_p = cloth_0.x[t,p]
        v_p = cloth_0.v[t,p]
        v_p += cloth_0.grasp_flag[p] * (v_input[int(t / max_step * v_input_num)] - cloth_0.v[t,p])
        C_p = cloth_0.C[t,p]
        F_p = cloth_0.F[t,p]
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            grid_m[base + offset] += weight * cloth_0.m  # mass conservation
            grid_v_in[base + offset] += weight * cloth_0.m * v_p + weight * cloth_0.m * C_p @ dpos # grid_v is acatually momentum, not velocity
            grid_v_in[base + offset] -= weight * 4 * dt * inv_dx * inv_dx * cloth_0.vol * P @ dF_dC @ dpos
            grid_v_in[base + offset].z += weight * dt * cloth_0.m * -9.8  
...
@ti.kernel
def Grid_To_Particle(t: ti.i32, cloth_0:ti.template()):
    for p in range(cloth_0.n):
        x_p = cloth_0.x[t,p]
        ...
        cloth_0.set_C(t + 1,next_C,p)
        cloth_0.set_v(t + 1,new_v,p)
        x_p += dt * cloth_0.v[t,p]
        cloth_0.set_x(t + 1,x_p,p)
    
    for p in range(cloth_0.pio_n):
        cloth_0.pio_x[t + 1,p] = cloth_0.x[t + 1,int(p*cloth_0.pio_scale)]
        cloth_0.pio_x_t[p] = cloth_0.x[t + 1,int(p*cloth_0.pio_scale)]
        cloth_0.pio_v[t + 1,p] = cloth_0.v[t + 1,int(p*cloth_0.pio_scale)]
...
@ti.ad.grad_replaced
def substep(counter, cloth_0, hanger):
    Reset()
    Particle_To_Grid(counter, cloth_0)
    Grid_Operations(hanger)
    Grid_To_Particle(counter, cloth_0)

@ti.ad.grad_for(substep)
def substep_grad(counter, cloth_0, hanger):
    Reset()
    Particle_To_Grid(counter, cloth_0)
    Grid_Operations(hanger)
    
    Grid_To_Particle.grad(counter, cloth_0)
    Grid_Operations.grad(hanger)
    Particle_To_Grid.grad(counter, cloth_0)
...
@ti.kernel
def compute_this_loss(cloth_0:ti.template(), target:ti.template(),step: ti.i32):
    for p in range(cloth_0.pio_n):
        dist = ti.math.length(cloth_0.pio_x[step, p] - target[p])
        loss_field[step] += dist
    #loss_field[step] = accu_loss
    
def compute_minimum_index(cloth_0,target,minimum_loss, minimu_index):
    for step in range(max_step):
        compute_this_loss(cloth_0, target, step)
    for step in range(max_step):
...
@ti.kernel
def compute_loss(cloth_0:ti.template(), v_input: ti.template(), minimum_index: ti.i32):

    for p in range(10):
        #dist = ti.math.length(cloth_0.pio_x[minimum_index, p] - ti.Vector([0.5, 0.5, 0.8]))
        dist = (cloth_0.x[minimum_index, p][2] - 0.6) ** 2
        loss[None] += dist 
        loss_position[None] += dist 
    # for step in range(max_step-2):
    #     if (cloth_0.pio_x[step, 0][1] - 0.6) ** 2 < 0.004:
    #         loss[None] += ((cloth_0.pio_x[step, 0][1] - 0.6) ** 2) ** 0.5
    #         loss_velocity[None] +=  ((cloth_0.pio_x[step, 0][1] - 0.6) ** 2) ** 0.5
    #         if cloth_0.pio_x[step, 0][1] - 0.5 < 0:
    #             loss[None] += 5 * ((cloth_0.pio_x[step, 0][1] - 0.5) ** 2) ** 0.5
    #             loss_velocity[None] += 5 *  ((cloth_0.pio_x[step, 0][1] - 0.5) ** 2) ** 0.5
...
def main():
    ...
    cloth_0.initialize(x_np, normal_np, mesh_np)
    
    hanger = Hanger('./Hanger',[0.5, 0.5, 0.5], (0.4, 0.4, 0.4))
    #path = Path('./target_path', target_num, (0.5,0.8,0.5), cloth_0.n, cloth_0.pio_n, cloth_0.nx)
    #path.initialize()
    ...
    while MPC_iter_num < max_MPC_iter:
        if MPC_iter_num == 0:
            ...
            cloth_0.mpc_initialize(max_step)
        ...
        while iter_num <= max_iter:
            ...
            for t in range(max_step - 1):
                substep(t, cloth_0, hanger)
                
                if t % 100 == 0: 
                    render(t,[cloth_0], [], [], camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ)
            ...
            #     #minimum_index = compute_minimum_index(cloth_0,pio_target,1000.0,0)
            #     #r_minimum_index[MPC_iter_num*(max_iter-1) + iter_num -1] = minimum_index
            #     compute_loss(cloth_0, v_input,max_step-1)
            ...
            # #print('end effector:', int(cloth_0.pio_x[max_step-1,0][1] * 100)/100)
