...
# Object
center, dimension = ti.Vector([0.5, 0.5, 0.4]), ti.Vector([0.25, 0.25, 0.0001])
axis_angle = ti.Vector([0 / 180 * pi, 1.0, 0.0, 0.0])
e_radius, total_mass, pho = 0.001, 4e4, 4.0
color = (0.5,0.5,0.8)
...
def Particle_To_Grid_Quad(obj:ti.template()):
    ...
    for p in obj.x_quad:
        ...
        d1 = (p3 - p1) * (1 / e_radius * 0.5)
        d2 = (p2 - p1) * (1 / e_radius * 0.5)
        ...
        P, F_elastic, F_plastic = compute_stress(R_inv @ F_p,mu,lam)
        #P = R @ (P @ (F_plastic.transpose().inverse()))
        #P = R @ P

        #F_T = F_plastic.inverse().transpose() @ F_p.transpose()
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            grid_m[base + offset] += weight * m  # mass conservation
            grid_v[base + offset] += weight * m * v_p + weight * m * C_p @ dpos # grid_v is acatually momentum, not velocity
            grid_v[base + offset] -= weight * 4 * dt * inv_dx * inv_dx * vol * R @ (P @ F_T @ dpos)
            #grid_v[base + offset].z += weight * dt * m * -9.8
