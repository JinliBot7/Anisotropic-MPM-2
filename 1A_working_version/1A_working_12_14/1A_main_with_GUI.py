...
# Object
center, dimension = ti.Vector([0.5, 0.5, 0.4]), ti.Vector([0.25, 0.25, 0.001])

e_radius, total_mass, pho = 0.001, dimension[0] * dimension[1] * dimension[2], 1.0
color = (0.5,0.5,0.8)
obj_0 = Obj(center, dimension, e_radius, total_mass, pho, color)



# Static Boundary
center, quaternion, radius, height, friciton, resolution, e_radius = ti.Vector([0.4, 0.5, 0.3]), ti.Vector([0.0, 1.0, 0.0, 0.0]), 0.02, 0.02, 0.3, 32, 0.001
...
# # Manipulator
# center, quaternion, radius, height, friciton, resolution, e_radius = ti.Vector([0.3, 0.5, 0.1]), ti.Vector([0.0, 1.0, 0.0, 0.0]), 0.005, 0.04, 0.3, 32, 0.001
# speed = ti.Vector([0.0, 0.0, 0.0])
# gripper_0 = Cylinder(center, speed, quaternion, radius, height, friciton, resolution, e_radius, (0.5, 0.5, 0.5),dx)
...
def Particle_To_Grid(obj:ti.template()):
    ...
    for p in obj.x:
        ...
        # F_T = F_p.transpose()
        # F_inv_T = F_T.inverse()
        # P_iso = mu * (F_p - F_inv_T) + lam * ti.log(J) * F_inv_T
        
        F_p = obj.F[p]
        F_T = F_p.transpose()
        P, F_elastic, F_plastic = compute_stress(obj.F[p],mu,lam)
        
        P = P @ F_plastic.transpose().inverse()

        obj.set_F(F_elastic,p)
        obj.set_F_plastic(F_plastic,p)
...
# @ti.kernel   
# def Clamping(obj:ti.template()):
#     for p in obj.F:
#         U, sig, V = ti.svd(obj.F[p])
#         for d in ti.static(range(3)):
#             new_sig = min(max(sig[d, d], 0.9), 2.0)  # Plasticity
#             sig[d,d] = new_sig
        
#         F_elastic = U @ sig @ V
#         obj.F_plastic[p] = F_elastic.inverse() @ obj.F[p]
#         obj.F[p] = F_elastic
...
def main():
    ...
    while window.running:
        ...
        #update_boundary(boundary_0)
        #update_boundary(boundary_1)
        
        # new_x_value = window.GUI.slider_float('x', old_x_value, -1, 1)
        # new_y_value = window.GUI.slider_float('y', old_y_value, -1, 1)
        # new_z_value = window.GUI.slider_float('z', old_z_value, -1, 1)
        # for gripper in gripper_list:
        #     Update_gripper(gripper, new_x_value,new_y_value, new_z_value)
