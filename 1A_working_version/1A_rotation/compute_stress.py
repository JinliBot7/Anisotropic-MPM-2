...
@ti.func
def compute_stress(F:ti.types.matrix(3,3,ti.f32), mu: ti.f32, lam: ti.f32):
    Q, R = QR3(F)
    Q_T = Q.transpose()
    R_T = R.transpose()
    Q_T_inv = Q_T.inverse()
...
def QR3(Mat:ti.types.matrix(3,3,ti.f32)) :
    a1 = ti.Matrix([Mat[0,0],Mat[1,0],Mat[2,0]])
    a2 = ti.Matrix([Mat[0,1],Mat[1,1],Mat[2,1]])
    a3 = ti.Matrix([Mat[0,2],Mat[1,2],Mat[2,2]])
    u1 = a1
    e1 = u1/u1.norm(1e-6)
