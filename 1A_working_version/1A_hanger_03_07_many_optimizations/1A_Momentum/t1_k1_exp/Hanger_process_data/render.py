...
def set_render():
    ...
    window = ti.ui.Window("MPM 3D", res, vsync=False, show_window = True)
...
def render(obj_list, manipulator_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ):
    ...
    for i in range(len(obj_list)):
        ...
        #scene.mesh(obj_list[i].x_t, indices = obj_list[i].mesh_indices, color = obj_list[i].color)
    for i in range(len(manipulator_list)):
        ...
        #scene.particles(manipulator_list[i].x, radius = 0.002, color = manipulator_list[i].color)
    ...
    #scene.particles(target, radius = 0.004, color = (0.0, 0.0, 0.5))
