...
def record_data(iteration,loss, r_loss, r_v_input, r_grad):
...
def main():
    ...
    dv_nda = ti.ndarray(dtype=ti.math.vec3, shape=(max_iter, v_input_num)) # For Gradient Decent with Momentum
    ...
    while iter_num < max_iter:
        ...
        for i in range(v_input_num):
            for j in range(3):
                if iter_num == 0:
                    dv = k * loss[None] * grad[i][j] / grad_square
                else:
                    dv = k * (Beta * dv_nda[iter_num, i][j] + (1 - Beta) * loss[None] * grad[i][j] / grad_square) / (1 - Beta ** iter_num)
                if dv >=0:
                    v_input[i][j] -= ti.min(dv, 0.1)
                    dv_nda[iter_num, i][j] = -ti.min(dv, 0.1)
                else:
                    v_input[i][j] -= ti.max(dv, -0.1)
                    dv_nda[iter_num, i][j] = -ti.max(dv, -0.1)
        
        print('iteration: ', iter_num)
        # print('grad=', grad[0])
        print('dv',dv_nda[iter_num,0])
        print('loss=', loss[None])
        # print('v[0]=', v_input[0])
        
        
        record_v_input(iter_num, v_input, grad, r_v_input, r_grad)
        record_data(iter_num,loss[None], r_loss, r_v_input, r_grad)
