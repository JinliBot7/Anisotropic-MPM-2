...
def render(counter, obj_list, manipulator_list, path_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,ball_center, ball_radius):
    ...
    for i in range(len(obj_list)):
        ...
        scene.particles(obj_list[i].pio_x_t, radius = obj_list[i].e_radius * 1.1  , color = (0.0, 0.5, 0.0))
        #pass
    ...
    for i in range(len(path_list)):
        scene.particles(path_list[i].x, radius = obj_list[0].e_radius * 0.7, per_vertex_color = path_list[i].colors)
        scene.particles(path_list[i].pio_x, radius = obj_list[0].e_radius * 1.1 * 0.7, color = (0.0, 0.5, 0.0))
