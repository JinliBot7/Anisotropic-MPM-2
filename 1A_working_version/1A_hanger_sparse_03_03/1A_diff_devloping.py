...
# initialize object
center, dimension = ti.Vector([0.5, 0.6, 0.4]), ti.Vector([0.4, 0.4, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def ini_v_input():
    for p in v_input:
        ...
        v_input[p] = [0.01, -v_input_ini_max, -v_input_ini_max]
...
def main():
    ...
    while iter_num <= max_iter:
        for t in range(max_step - 1):
            ...
            if t % 10 == 0: 
                render(t,[obj], [hanger], camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,Circle_Center,Circle_Radius)
        ...
        for i in range(v_input_num):
            for j in range(3):
                if learning_rate * grad[i][j] >=0:
                    v_input[i][j] -= ti.min(learning_rate * grad[i][j], 0.2)
                elif learning_rate * grad[i][j] <0:
                    v_input[i][j] -= ti.max(learning_rate * grad[i][j], -0.2)
