...
def compute_stress(F, mu, lam):
    ...
    if r33 < 1:
        f_n = k / ((r11 * r22) * (r33 - 1) ** 2 + + 1e-10)
        f_f = gamma / ((r11 * r12) * (r13 ** 2 + r23 ** 2) ** 0.5 + 1e-10)
        cf = 0.0
        if f_f > cf * f_n:
            r13 = R[0,2] = cf * f_n / (f_f * r13 + 1e-10)
            r23 = R[1,2] = cf * f_n / (f_f * r23 + 1e-10)
    ...
    # nan_check = ti.math.isnan(F_elastic)
    # sum_nan = nan_check[0,0] + nan_check[0,1] + nan_check[0,2] + \
    #         nan_check[1,0] + nan_check[1,1] + nan_check[1,2] + \
    #         nan_check[2,0] + nan_check[2,1] + nan_check[2,2] 
