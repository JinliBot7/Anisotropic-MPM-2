...
def set_render():
    ...
    camera = ti.ui.Camera()
    scale = 0.9
    camera.position(0.9*scale, 0.9*scale, 0.7*scale)
    camera.lookat(0, 0, 0)
    camera.up(0, 0, 1)
...
def render(obj_list, manipulator_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,ball_center, ball_radius, target):
