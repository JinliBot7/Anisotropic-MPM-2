...
ti.init(ti.cuda,device_memory_fraction = 0.9, offline_cache = True, kernel_profiler = False)
...
# Object
center, dimension = ti.Vector([0.5, 0.5, 0.37]), ti.Vector([0.3, 0.3, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
axis_angle = ti.Vector([-180 / 180 * pi, 1.0, 0.0, 0.0]) # angle and rotation axis
# center, dimension = ti.Vector([0.5, 0.85, 0.48]), ti.Vector([0.25, 0.25, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def Particle_To_Grid(obj:ti.template(), counter:ti.f32):
    ...
    for p in obj.x:
        ...
        # if counter < 40000:
            # v_p += obj.is_grasping_point(p) * (ti.Vector([0.0,-0.1,0.0]) - obj.v[p])
        # elif counter < 15000:
        ...
        if counter > 5e3 and counter < 8e4:
            if (obj.is_grasping_point(p)):
                error = ti.Vector([0.375, 0.55, 0.7]) - obj.x[0]
                v_p = [speed * error[0], speed * error[1], speed * error[2]]

        

        if p == 0 and counter % 1000 == 0:
            print(counter,x_p)
        ...
        # obj.energy[0] +=  0.5 * m * v_p[0] ** 2 + 0.5 * m * v_p[1] ** 2 + 0.5 * m * v_p[2] ** 2
    ...
    for p in boundary.sdf:
        ...
        if grid_m[grid_index] != 0:
            ...
            if v_proj < 0:
                ...
                grid_v[grid_index] -= v_normal * boundary.sdf[p] * 1
...
def main():
    ...
    for counter in range(100000):
        ...
        if counter % 100 == 0: 
            ...
            window.show()
            #print('counter: ', counter * 5e-5, 'energy: ',obj_0.energy[0])
            #print(counter)
