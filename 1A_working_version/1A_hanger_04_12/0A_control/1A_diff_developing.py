...
center, dimension = ti.Vector([0.5, 0.8, 0.4]), ti.Vector([0.27, 0.27, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
    ...
    for p in range(obj.n):
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            if grid_m[base + offset] != 0:
                g_v = grid_v_out[base + offset] / (grid_m[base + offset] + 1e-14)
            
            # if p == 0:
            #     print(grid_m[base + offset]* 1e10)
...
def ini_v_input(obj:ti.template(), target: ti.template()):
    ...
    # for p in range(obj.pio_n):
    #     ini_v += (target[p] - obj.pio_x[0, p]) / (dt * max_step * obj.pio_n)
    for p in v_input:
        v_input[p] = ti.Vector([0.0, -0.1, 0.0])
...
#             pass
#         else:
#             obj_relative_dist = obj.pio_x[max_step - 1, p] - obj.pio_x[max_step - 1, 0]
#             target_relative_dist = target[p] - target[0]
#             dist = ti.math.length(obj_relative_dist - target_relative_dist)
...
#     # for p in v_input:
#     #     for i in range(3):
#     #             vij_up = 0.025 * ti.pow(10, 10 * ti.abs(v_input[p][i]) - 2)
#     #             loss[None] += vij_up
#     #             loss_velocity[None] += vij_up
...
def compute_loss(obj:ti.template(), target:ti.template(), v_input: ti.template()):
    for p in range(obj.pio_n):
        ...
        loss_standard[None] += dist
        # if p < 45:
        #     loss_position[None] += 10 * ti.pow(2, -(100 * obj.pio_x[max_step - 1, p][1] - 50))
        #     loss[None] += 10 * ti.pow(2, -(100 * obj.pio_x[max_step - 1, p][1] - 50))
    # for p in v_input:
    #     for i in range(3):
    #             vij_up = 0.025 * ti.pow(10, 10 * ti.abs(v_input[p][i]) - 2)
    #             loss[None] += vij_up
...
#     for p in range(obj.pio_n):
#         #dist = ti.math.length(obj.pio_x[max_step - 1, p] - target[p])
#         loss[None] += -obj.pio_x[max_step - 1, p][2] + ti.pow(2, -(100 * obj.pio_x[max_step - 1, p][1] - 50)) + ti.pow(2,100 * obj.pio_x[max_step - 1, p][1] - 90) + obj.pio_v[max_step - 1,p][1] ** 2
...
def main():
    ...
    r_loss_position = ti.field(dtype = ti.f32, shape = max_iter)
    r_loss_velocity = ti.field(dtype = ti.f32, shape = max_iter)
    ...
    while iter_num < max_iter:
        ...
        #visualization
        # if iter_num == 0 or (iter_num + 1) % 2 == 0:
        # try:
        #     os.mkdir(f'./vis/{iter_num}')
        # except FileExistsError:
        #     pass
        #     print('fiel alrady exist!')
        # video_manager = ti.tools.VideoManager(f'./vis/{iter_num}',framerate = 66.7, automatic_build=False)
        ...
        # #     if iter_num == 0 or (iter_num + 1) % 20 == 0:
        #     #if t % 100 == 0:
        ...
        #     for t in range(max_step - 1):
        #         substep(t, obj, hanger)
        #     gripper_dist =  (obj.x[max_step - 1, 5 * obj.nx + 5][0] - 0.5) ** 2
        #     compute_loss(obj,pio_target, v_input)
        #     #compute_collision_loss(gripper_dist)
        ...
        # print('iteration: ', iter_num,'loss:', loss[None], 'loss_pos:', loss_position[None], 'los_vel:', loss_velocity[None])
        # # #print('dv: ', v_input[0])
        ...
        # record_data(iter_num, r_loss,r_loss_standard, r_loss_position, r_loss_velocity, r_v_input, r_grad)
