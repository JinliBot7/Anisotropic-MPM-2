...
max_iter = 50 # max iteration in a MPC update
...
grid_v_out = vec() 
grid_m = scalar()
grid_collision = scalar() # for record collision in this step
grid_collision_force = vec()
block = ti.root.pointer(ti.ijk, (int(N_grid/4),int(N_grid/4),int(N_grid/4)))
pixel = block.dense(ti.ijk, (4,4,4))
pixel.place(grid_v_in,grid_v_out, grid_m,grid_collision,grid_collision_force)

loss, loss_standard, loss_position, loss_velocity = scalar(), scalar(), scalar(), scalar()
loss_field = scalar() # for minimum index
collision_count = scalar()
ti.root.place(loss, loss_standard,loss_position,loss_velocity,collision_count)
...
# initialize object
center, dimension = ti.Vector([0.5, 0.9, 0.4]), ti.Vector([0.3, 0.3, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def Particle_To_Grid(t: ti.i32, obj:ti.template()):
    for p in range(obj.n):
        ...
        v_p = obj.v[t,p]
        if t < max_step * 0.5:
            v_p += obj.grasp_flag[p] * (v_input[int(t / (max_step* 0.5) * v_input_num)] - obj.v[t,p])
        else:
...
def Grid_Operations(boundary:ti.template()):
    for i, j, k in grid_v_in:
        grid_collision[i,j,k] = 0.0
        grid_collision_force[i,j,k] = [0.0, 0.0, 0.0]
    ...
    for p in boundary.sdf:
        ...
        if grid_m[grid_index] != 0:
            ...
            v_proj = ti.math.dot(v_out,boundary.sdf_n[p])
            if v_proj < 0:
                ...
                grid_collision[grid_index] = 1.0
                grid_collision_force[grid_index] = -v_normal * boundary.sdf[p]
...
def Grid_To_Particle(t: ti.i32, obj:ti.template()):
    for p in range(obj.n):
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            # record collision of pio
            if obj.pio_flag[p] == 1.0 and grid_collision[base + offset] == 1.0:
                obj.pio_collision[t,p % obj.pio_nx * obj.pio_scale + p // obj.pio_nx * obj.nx * obj.pio_scale] = 1.0
                obj.pio_collision_force[t,p % obj.pio_nx * obj.pio_scale + p // obj.pio_nx * obj.nx * obj.pio_scale] += weight * grid_collision_force[base + offset]
...
def Reset():
    for i, j, k in grid_m:
        ...
        grid_collision.grad[i, j, k] = 0.0
        grid_collision_force.grad[i, j, k] = [0.0, 0.0, 0.0]
...
def ini_v_input():
    for p in range(v_input_num):
        v_input[p] = ti.Vector([0.0, -0.6,0.0])
...
@ti.kernel
def compute_loss(obj:ti.template(), target:ti.template(), v_input: ti.template(), minimum_index: ti.i32):

    for p in range(obj.pio_n):
        dist = ti.math.length(obj.pio_x[minimum_index, p] - target[p])
        loss[None] += obj.pio_coefficient[p] * dist / obj.pio_n
        loss_position[None] += obj.pio_coefficient[p] * dist / obj.pio_n
        
    # for t,p in obj.pio_collision:
    #     if obj.pio_collision[t,p] == 1.0:
    #         dist_vec = target[p] - obj.pio_x[t, p]
    #         #v_proj = dist_vec.dot(obj.pio_collision_force[t, p])
    #         v_proj = ti.math.dot(dist_vec,obj.pio_collision_force[t, p])
    #         if v_proj < 0:
    #             collision_count[None] += 1
    #             loss[None] += obj.pio_collision_force[t,p][0] ** 2 +  obj.pio_collision_force[t,p][1] ** 2 +  obj.pio_collision_force[t,p][2] ** 2
    #             loss_velocity[None]  += obj.pio_collision_force[t,p][0] ** 2 +  obj.pio_collision_force[t,p][1] ** 2 +  obj.pio_collision_force[t,p][2] ** 2
    
                
    # for t,p in obj.pio_collision:
    #         if obj.pio_collision[t,p] == 1:
    #             dist_vec = target[p] - obj.pio_x[t, p]
    #             v_proj = dist_vec.dot(obj.pio_collision_force[t, p])
    #             if v_proj < 0:
    #                 loss[None] -=  ti.math.length(target[p] - obj.pio_x[t, p]) / (collision_count[None] +1) # +1 to prevent numerical error
    #                 loss_velocity[None] -= ti.math.length(target[p] - obj.pio_x[t, p]) / (collision_count[None] +1)
            
    
    # for t,p in obj.particle_collision:
    #     if obj.particle_collision[t,p] == 1:
    #         loss[None] += 
...
def record_data(iteration, r_loss, r_loss_standard, r_loss_position, r_loss_velocity,  r_v_input, r_grad,r_minimum_index, obj):
    ...
    pio_collision_np = obj.pio_collision.to_numpy()
    ...
    np.save('./data/pio_collision.npy',pio_collision_np)
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            with ti.ad.Tape(loss=loss):
                ...
                compute_loss(obj,pio_target, v_input,max_step-1)
            ...
            if iter_num == 0:
                ...
                for i in range(v_input_num):
                    for j in range(3):
                        ...
                        #v_input[i][j] -= dv
                        if grad[i][j] > 0:
                            v_input[i][j] -= min(0.04,k*grad[i][j])
                        else:
                            v_input[i][j] -= max(-0.04,k*grad[i][j])
                        
                        if i == 0 and j == 1:
                            if grad[i][j] > 0:
                                print(min(0.04,k*grad[i][j]),k*grad[i][j],math.isnan(grad[i][j]))
                            else:
                                print(max(-0.04,k*grad[i][j]),k*grad[i][j],math.isnan(grad[i][j]))
                                
    
                        
            
            print('total_iter:', MPC_iter_num*(max_iter-1) + iter_num -1,'MPC_iter:',MPC_iter_num,'iter: ', iter_num,'loss:', loss[None], 'loss_pos:', loss_position[None], 'los_vel:', loss_velocity[None], 'collision_count:', collision_count[None])
            ...
            record_data(MPC_iter_num*(max_iter-1) + iter_num -1
                        , r_loss,r_loss_standard, r_loss_position, r_loss_velocity, r_v_input, r_grad, r_minimum_index,obj)
