    ...
    for p in boundary.sdf:
        ...
        if grid_m[grid_index] != 0:
            ...
            if v_proj < 0:
                ...
                grid_collision[grid_index] += 1.0 # actaully each grid is visited once
                grid_collision_force[grid_index] -= v_normal * boundary.sdf[p] / (grid_m[grid_index] + 1e-16)
    ...
    for p in range(obj.n):
        ...
        for offset in ti.static(ti.grouped(ti.ndrange(*neighbour))):
            ...
            if obj.pio_flag[p] == 1.0 and grid_collision[base + offset] == 1.0:
                pio_index = p % obj.pio_nx * obj.pio_scale + p // obj.pio_nx * obj.nx * obj.pio_scale
                obj.pio_collision[t,pio_index] = 1.0
                obj.pio_collision_force[t,pio_index] += weight * grid_collision_force[base + offset]
...
def compute_loss(obj:ti.template(), target:ti.template(), v_input: ti.template(), minimum_index: ti.i32):
    ...
    #             loss[None] += 0.0001 * (obj.pio_collision_force[t,p][0] ** 2 +  obj.pio_collision_force[t,p][1] ** 2 +  obj.pio_collision_force[t,p][2] ** 2)
    #             loss_velocity[None]  += 0.0001 * (obj.pio_collision_force[t,p][0] ** 2 +  obj.pio_collision_force[t,p][1] ** 2 +  obj.pio_collision_force[t,p][2] ** 2)
    ...
    for t,p in obj.pio_collision:
            if obj.pio_collision[t,p] == 1.0:
                ...
                v_proj = ti.math.dot(dist_vec,obj.pio_collision_force[t, p])
                if v_proj < 0:
                    dist = (dist_vec[0] ** 2 + dist_vec[1] ** 2 + dist_vec[2] ** 2) ** 0.5
                    loss[None] -= -v_proj *  obj.pio_coefficient[p] *  0.75 * dist / (collision_count[None] +1) # +1 to prevent numerical error
                    loss_velocity[None] -= -v_proj * obj.pio_coefficient[p] * 0.75 * dist / (collision_count[None] +1)

@ti.kernel
def collision_flag_compute(obj:ti.template(), target:ti.template())->ti.f32:
    # position collision loss
    return_flag = 0.0
    for t,p in obj.pio_collision:
        if obj.pio_collision[t,p] == 1.0:
            dist_vec = target[p] - obj.pio_x[t, p]
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            with ti.ad.Tape(loss=loss):
                ...
                #collision_flag = collision_flag_compute(obj,pio_target)
            ...
            if iter_num == 0:
                ...
                for i in range(v_input_num):
                    for j in range(3):
                        ...
                        v_input[i][j] -= dv
                        # if math.isnan(grad[i][j]) or math.isnan(dv):
                        #     print(i,j,grad[i][j],dv)
                        # if i == 0 and j ==0:
                        #     print(grad[i][j],dv)
