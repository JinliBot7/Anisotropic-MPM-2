...
# initialize object
center, dimension = ti.Vector([0.5, 0.5, 0.37]), ti.Vector([0.4, 0.4, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def Particle_To_Grid(t: ti.i32, obj:ti.template()):
    for p in range(obj.n):
        x_p = obj.x[t,p]
        #v_p = obj.v[t,p] + obj.grasp_flag[p] * (v_input[None] - obj.v[t,p])
        v_p = obj.v[t,p] + obj.grasp_flag[p] * (v_input[0] - obj.v[t,p])
        #v_p = obj.v[t,p] + obj.grasp_flag[p] * (v_input[int(t / max_step * v_input_num)] - obj.v[t,p])
        C_p = obj.C[t,p]
        F_p = obj.F[t,p]
...
def substep(counter, obj_0, hanger):
    ...
    #upate_target_point(counter, obj_0)
...
def main():
    ...
    for i in range(max_step - 1):
        ...
        if i % 10 == 0: 
            render(i,[obj], [hanger], [path], camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,Circle_Center,Circle_Radius)
