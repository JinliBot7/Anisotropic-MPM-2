...
center, dimension = ti.Vector([0.5, 0.5, 0.39]), ti.Vector([0.27, 0.27, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def Particle_To_Grid(obj:ti.template(), counter:ti.f32):
    ...
    for p in obj.x:
        ...
        error = ti.Vector([0.375, 0.6, 0.65]) - obj.x[0]
        error_length = ti.math.length(error)
        if p == 0 and counter % 100 == 0:
            print(counter, counter*dt, error_length)
        if counter > 6e3 and counter < 10e4:
...
def main():
    ...
    for counter in range(100000):
        ...
        if counter > 6000 and (counter - 6000) % 50 == 0:
            target_point_np = obj_0.pio_x.to_numpy()
            np.save(f'target_path/target_{counter - 6000}.npy',target_point_np)
            
            path_np = obj_0.x.to_numpy()
            np.save(f'target_path/path_{counter - 6000}.npy',path_np)
