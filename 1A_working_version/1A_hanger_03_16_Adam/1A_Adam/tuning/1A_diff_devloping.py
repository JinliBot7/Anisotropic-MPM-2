...
def record_data(iteration, r_loss, r_loss_standard, r_v_input, r_grad, tuning_iteration):
    ...
    np.save(f'./data/v_input_{tuning_iteration}.npy',v_input_np)
    np.save(f'./data/loss_{tuning_iteration}.npy',loss_np)
    np.save(f'./data/loss_standard_{tuning_iteration}.npy',loss_standard_np)
    np.save(f'./data/grad_{tuning_iteration}.npy',grad_np)
...
def main():
    ...
    parameter_list = []
    for tuning_iteration in range(1000):
        learning_rate = 10 ** (np.random.rand() - 2)
        loss_exp_base = 10 ** (2 * np.random.rand())
        obj.set_pio_coe(loss_exp_base)
            ...
            for i in range(v_input_num):
                for j in range(3):
                    if iter_num == 0:
                        ...
                        dv = learning_rate * V_hat / (S_hat**0.5 + 1e-8)
                        # threshold
                        scale, dv = threshold(dv)
                        V_nda[iter_num, i][j] /= scale
                        v_input[i][j] -= dv        
            print('tuning iter:',tuning_iteration, 'iteration: ', iter_num,'loss:', loss[None])
            record_v_input(iter_num, v_input, grad, r_v_input, r_grad)
            record_data(iter_num, r_loss,r_loss_standard, r_v_input, r_grad, tuning_iteration)
            iter_num += 1
        
        parameter_list.append([f'k = {learning_rate}, a = {loss_exp_base}'])
        parameter_list_np = np.array(parameter_list)
        np.save('./data/parameter.npy',parameter_list_np)
