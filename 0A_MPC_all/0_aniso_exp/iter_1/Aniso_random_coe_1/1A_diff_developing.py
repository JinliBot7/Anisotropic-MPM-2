...
max_iter = 400 # max iteration in a MPC update
...
# initialize object
center, dimension = ti.Vector([0.5, 0.8, 0.6]), ti.Vector([0.25, 0.25, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def ini_v_input():
    for p in range(v_input_num):
        rd0 = 0.2 * (ti.random(float) - 0.5)
        rd1 = 0.2 * (ti.random(float) - 0.5)
        rd2 = 0.2 * (ti.random(float) - 0.5)
        v_input[p] = ti.Vector([rd0, rd1, rd2])
...
def update_pio_coe(obj:ti.template(), target:ti.template(),dist_nda: ti.types.ndarray(), iter_num: ti.i32,r_pio_coefficient: ti.template(),r_pio_dist:ti.template(),r_pio_dist_hat:ti.template()):
    ...
    #     dist_nda[1, p] = beta_loss * dist_nda[0, p] + (1 - beta_loss) * dist
    #     dist_nda_hat = dist_nda[1, p] / (1 - beta_loss ** iter_num)
    #     dist_nda[0, p] -= dist_nda[0, p]
    #     dist_nda[0, p] +=  dist_nda[1, p] 
    #     loss_all[None] += ti.pow(10,10*dist_nda_hat)
    #     r_pio_dist[iter_num,p] = ti.math.length(obj.pio_x[max_step-1, p] - target[p])
    #     r_pio_dist_hat[iter_num,p] = dist_nda_hat


    # for p in range(obj.pio_n):
    #     dist_nda_hat = dist_nda[1, p] / (1 - beta_loss ** iter_num)
    #     obj.pio_coefficient[p] = ti.pow(10, 10*dist_nda_hat) / loss_all[None]
    #     r_pio_coefficient[iter_num,p] = obj.pio_coefficient[p]
    

    for p in range(obj.pio_n):
        obj.pio_coefficient[p] = ti.random()
        r_pio_coefficient[iter_num,p] = obj.pio_coefficient[p]
...
def record_data(iteration, r_loss, r_loss_standard, r_loss_position, r_loss_velocity,  r_v_input, r_grad,r_minimum_index,r_pio_coefficient,r_pio_dist,r_pio_dist_hat,opt_n:ti.i32):
    ...
    np.save(f'./data/v_input_{opt_n}.npy',v_input_np)
    np.save(f'./data/loss_{opt_n}.npy',loss_np)
    np.save(f'./data/loss_standard_{opt_n}.npy',loss_standard_np)
    np.save(f'./data/loss_position_{opt_n}.npy',loss_position_np)
    np.save(f'./data/loss_velocity_{opt_n}.npy',loss_velocity_np)
    np.save(f'./data/minimum_index_np_{opt_n}.npy',r_minimum_index_np)
    np.save(f'./data/grad_{opt_n}.npy',grad_np)
    np.save(f'./data/pio_coe_{opt_n}.npy',pio_coe_np)
    np.save(f'./data/pio_dist_{opt_n}.npy',pio_dist_np)
    np.save(f'./data/pio_dist_hat_{opt_n}.npy',pio_dist_hat_np)
...
def main():
    ...
    r_pio_coefficient = ti.field(dtype = ti.f32, shape =  (500,obj.pio_n))
    r_pio_dist_hat = ti.field(dtype = ti.f32, shape =  (500,obj.pio_n))
    r_pio_dist = ti.field(dtype = ti.f32, shape =  (500,obj.pio_n))
    ...
    dist_nda = ti.ndarray(dtype=ti.f32, shape=(2, obj.pio_n))
    ...
    while opt_n <= 50:
        ...
        while MPC_iter_num < max_MPC_iter:
            ...
            while iter_num <= max_iter:
                ...
                print('opt_n:', opt_n,'iter: ', iter_num,'loss:', loss[None], 'loss_pos:', loss_position[None])
                ...
                record_data(MPC_iter_num*(max_iter-1) + iter_num -1
                            , r_loss,r_loss_standard, r_loss_position, r_loss_velocity, r_v_input, r_grad, r_minimum_index,r_pio_coefficient,r_pio_dist,r_pio_dist_hat,opt_n)
                
                iter_num += 1
                
                if loss_position[None] <= 2:
                    break
