...
@ti.kernel
def update_pio_coe(obj:ti.template(), target:ti.template(),loss_nda: ti.types.ndarray(), iter_num: ti.i32,r_pio_coefficient: ti.template(),r_pio_loss:ti.template()):
    for p in range(obj.pio_n):
        dist = obj.pio_coefficient[p] * ti.math.length(obj.pio_x[max_step-1, p] - target[p])
        
        loss_nda[1, p] = beta_loss * loss_nda[0, p] + (1 - beta_loss) * dist
        loss_nda_hat = loss_nda[1, p] / (1 - beta_loss ** iter_num)
        loss_nda[0, p] -= loss_nda[0, p]
        loss_nda[0, p] +=  loss_nda[1, p] 
        loss_all[None] += ti.pow(2,200*loss_nda_hat)
        r_pio_loss[iter_num,p] = ti.math.length(obj.pio_x[max_step-1, p] - target[p])


    for p in range(obj.pio_n):
        obj.pio_coefficient[p] = ti.pow(2,200*loss_nda[1, p] / (1 - beta_loss ** iter_num)) / loss_all[None]
...
def record_data(iteration, r_loss, r_loss_standard, r_loss_position, r_loss_velocity,  r_v_input, r_grad,r_minimum_index,r_pio_coefficient,r_pio_loss):
    ...
    loss_pio_np = r_pio_loss.to_numpy()
    ...
    np.save('./data/pio_loss.npy',loss_pio_np)
...
def main():
    ...
    r_pio_loss = ti.field(dtype = ti.f32, shape =  (500,obj.pio_n))
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            update_pio_coe(obj, pio_target, loss_nda, iter_num,r_pio_coefficient,r_pio_loss)
            ...
            record_data(MPC_iter_num*(max_iter-1) + iter_num -1
                        , r_loss,r_loss_standard, r_loss_position, r_loss_velocity, r_v_input, r_grad, r_minimum_index,r_pio_coefficient,r_pio_loss)
