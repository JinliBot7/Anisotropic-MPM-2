...
def compute_stress(F:ti.template(), mu, lam):
    ...
    if r33 < 1:
        f_n = k / ((r11 * r22) * (r33 - 1) ** 2 + 1e-8)
        f_f = gamma / ((r11 * r12) * (r13 ** 2 + r23 ** 2) ** 0.5 + 1e-8)
        cf = 0.0
        if f_f > cf * f_n:
            r13 = R[0,2] = cf * f_n / (f_f * r13 + 1e-8)
            r23 = R[1,2] = cf * f_n / (f_f * r23 + 1e-8)
...
def QR3(Mat:ti.template()) :
    ...
    u1 = a1
    e1 = u1/(get_norm(u1) + 1e-8)
    u2 = a2 - projection(u1,a2)
    e2 = u2/(get_norm(u2) + 1e-8)
    u3 = a3 - projection(u1,a3)-projection(u2,a3)
    e3 = u3/(get_norm(u3) + 1e-8)
