...
grid_v_out = vec() 
grid_m = scalar()
grid_collision_flag = scalar()
block = ti.root.pointer(ti.ijk, (int(N_grid/4),int(N_grid/4),int(N_grid/4)))
pixel = block.dense(ti.ijk, (4,4,4))
pixel.place(grid_v_in,grid_v_out, grid_m, grid_collision_flag)

loss, loss_standard, loss_position, loss_velocity, loss_all, pio_all, loss_collision = scalar(), scalar(), scalar(), scalar(), scalar(), scalar(), scalar()
loss_field = scalar()
ti.root.place(loss, loss_standard,loss_position,loss_velocity, loss_all, pio_all, loss_collision)
    ...
    for p in boundary.sdf:
        ...
        if grid_m[grid_index] != 0:
            ...
            if v_proj < 0:
                ...
                loss_collision[None] += 1e3 * grid_m[grid_index] 
                loss[None] += 1e3 * grid_m[grid_index]
...
def update_pio_coe(obj:ti.template(), target:ti.template(),dist_nda: ti.types.ndarray(), iter_num: ti.i32,r_pio_coefficient: ti.template(),r_pio_dist:ti.template(),r_pio_dist_hat:ti.template()):
    for p in range(obj.pio_n):
        ...
        loss_all[None] += ti.pow(3,10*dist_nda_hat)
    ...
    for p in range(obj.pio_n):
        dist_nda_hat = dist_nda[1, p] / (1 - beta_loss ** iter_num)
        obj.pio_coefficient[p] = ti.pow(3, 10*dist_nda_hat) / loss_all[None]
...
def main():
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            print('iter: ', iter_num,'loss:', loss[None], 'loss_pos:', loss_position[None], 'loss_colli:', loss_collision[None])
