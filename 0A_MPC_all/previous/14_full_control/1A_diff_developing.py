...
def compute_loss(obj:ti.template(), hanger: ti.template(), target:ti.template(),pio_hanger_vec_target:ti.template(), v_input: ti.template(), minimum_index: ti.i32):
    ...
    for h in range(hanger.n_sdf):
        for o in range(obj.pio_n):
            # diff =  ti.math.length(obj.pio_hanger_vec[o,h] - pio_hanger_vec_target[o,h])
            # loss[None] += diff
            
            diff =  obj.pio_hanger_vec[o,h] - pio_hanger_vec_target[o,h]
            
            sign_0 = obj.pio_hanger_vec[o,h][0] * pio_hanger_vec_target[o,h][0]
            sign_1 = obj.pio_hanger_vec[o,h][1] * pio_hanger_vec_target[o,h][1]
            sign_2 = obj.pio_hanger_vec[o,h][2] * pio_hanger_vec_target[o,h][2]
            
            if sign_0 <= 0:
                loss[None] += (diff[0] ** 2) ** 0.5
            if sign_1 <= 0:
                loss[None] += (diff[1] ** 2) ** 0.5
            if sign_2 <= 0:
                loss[None] += (diff[2] ** 2) ** 0.5
            #loss[None] += (diff[0] ** 2 + diff[1] ** 2 + diff[2] ** 2) 


            #     sign = obj.pio_hanger_vec[o,h][i] * pio_hanger_vec_target[o,h][i]
                # if sign <= 0:
                #     loss[None] += 1000 * (diff[i] ** 2) ** 0.5
                #     loss_position[None] += 1000 * (diff[i] ** 2) ** 0.5
                # else:
                #     loss[None] += (diff[i] ** 2) ** 0.5
