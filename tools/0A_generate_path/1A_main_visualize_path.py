...
center, dimension = ti.Vector([0.5, 0.5, 0.37]), ti.Vector([0.27, 0.27, 0.0005]) # dimension changes shape. if dimension[i] < e_radius, single layer
...
def main():
    ...
    while counter < 142:
        ...
        path = Path('./target_path', 1000+counter * 500, (0.5,0.8,0.5), obj_0.n, obj_0.pio_n, obj_0.nx)
        print(1000 + counter * 500)
        path.initialize()
        ...
        render(obj_list, boundary_list, [path] ,camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,Circle_Center,Circle_Radius)
