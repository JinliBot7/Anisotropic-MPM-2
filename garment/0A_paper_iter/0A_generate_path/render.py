...
def set_render():
    ...
    # camera.lookat(0, 0, 0)
    scale = 1.0
    camera.position(1.2*scale, 0.6*scale, 0.6*scale)
...
def render(obj_list, manipulator_list, path_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ,ball_center, ball_radius):
    ...
    for i in range(len(obj_list)):
        scene.particles(obj_list[i].x, radius = obj_list[i].e_radius * 0.5  , per_vertex_color = obj_list[i].colors)
        scene.particles(obj_list[i].pio_x, radius = obj_list[i].e_radius * 1.1 * 0.5  , color = (0.0, 0.5, 0.0))
    ...
    for i in range(len(path_list)):
        ...
        scene.particles(path_list[i].pio_x, radius = obj_list[0].e_radius * 0.5  , color = (0.0, 0.5, 0.0))
