...
def render(counter, obj_list, manipulator_list, path_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ, line, pio_list, real_point_list,kf_count):
    ...
    for i in range(196):
        line[int(i * 2)] = [pio_list[i,kf_count,0], pio_list[i,kf_count,1], pio_list[i,kf_count,2]]
        line[int(i * 2 + 1)] = [real_point_list[i,kf_count,0], real_point_list[i,kf_count,1], real_point_list[i,kf_count,2]]
        
    scene.lines(line, 3, color=colorX)
