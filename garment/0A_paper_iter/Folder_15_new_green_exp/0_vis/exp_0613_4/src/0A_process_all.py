...
#for n_frame in range(44,45,1):
    vert = np.load(f'../../../0A_exp_data/{exp_name}/exp_{exp_num}/vert/vert_{n_frame}.npy') * 100
    uv = np.load(f'../../../0A_exp_data/{exp_name}/exp_{exp_num}/uv/uv_{n_frame}.npy')
    bgr = cv.imread(f'../../../0A_exp_data/{exp_name}/exp_{exp_num}/rgb/rgb_img_{n_frame}.jpg')
    ...
    for i in range(vert.shape[0]):
        ...
        if h > 58 and h < 86 and s > 109 and s < 255 and v > 21 and v < 255:
            roi_flag[i] = 1.0
            #print(i,v)
    ...
    np.save(f'../../../0A_exp_data/{exp_name}/exp_{exp_num}/pcd/x_{n_frame}.npy',X_s_3d.astype('float32'))
    np.save(f'../../../0A_exp_data/{exp_name}/exp_{exp_num}/pcd/color_{n_frame}.npy',color.astype('float32'))

#%%
# fig = go.Figure(data=[go.Scatter3d(x=X_g[:,0], y=X_g[:,1], z=X_g[:,2],
#                                     mode='markers',marker=dict(
#         size=2, color = color            # set color to an array/list off desired values
#         # opacity=0.8
#     ))])
