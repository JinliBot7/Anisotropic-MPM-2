...
def set_render():
    ...
    camera.position(1.2*scale, 0.45*scale, 0.5*scale)
...
def render(counter, obj_list, manipulator_list, path_list, real_path_list, camera, scene, canvas, window, axisX, axisY, axisZ, colorX, colorY, colorZ):
    ...
    for i in range(len(obj_list)):
        ...
        #scene.particles(obj_list[i].pio_x_t, radius = obj_list[i].e_radius * 0.8  , color = (0.8, 0.8, 0.5))
        #pass
    ...
    for i in range(len(real_path_list)):
        scene.particles(real_path_list[i].x, radius = obj_list[0].e_radius * 0.5, per_vertex_color = real_path_list[i].colors)
    ...
    for i in range(len(path_list)):
        ...
        #scene.particles(path_list[i].pio_x, radius = obj_list[0].e_radius * 0.8, color = (0.8, 0.8, 0.5))
