...
def main():
    ...
    center, axis_angle = ti.Vector([0.5, 0.5, z_rnd]), ti.Vector([rot_rnd / 180 * pi, 1.0, 0.0, 0.0])
    ...
    while n_sample < 2000:
        ...
        z_rnd = np.random.uniform(low = 0.4, high = 0.45)
        rot_rnd = np.random.uniform(low = -180, high = -100)
        center, axis_angle = ti.Vector([0.5, 0.5, z_rnd]), ti.Vector([rot_rnd / 180 * pi, 1.0, 0.0, 0.0])
        obj_0.ini_random(center, axis_angle)
