...
def compute_obj_center(obj:ti.template()):
    for p in range(obj.pio_n):
        ...
        if obj.pio_x[max_step - 1, p][2] < 0.35:
            loss_position[None] += 2 * (0.35 - obj.pio_x[max_step - 1, p][2])
            loss_position_count[None] += 1
        elif obj.pio_x[max_step - 1, p][2] > 0.5:
            loss_position[None] += 2 * (obj.pio_x[max_step - 1, p][2] - 0.5)
            loss_position_count[None] += 1
@ti.kernel
...
def main():
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            print('iter: ', iter_num,'loss_ave:', loss[None] - loss_position[None] / loss_position_count[None] - loss_velocity[None], 'loss_upper_lower:', loss_position[None] / loss_position_count[None], 'loss_ef: ',loss_velocity[None])
