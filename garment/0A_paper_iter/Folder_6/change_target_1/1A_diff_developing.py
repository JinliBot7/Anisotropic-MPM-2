...
grid_v_out = vec() 
grid_m = scalar()
grid_m_target = scalar()
block = ti.root.pointer(ti.ijk, (int(N_grid/4),int(N_grid/4),int(N_grid/4)))
pixel = block.dense(ti.ijk, (4,4,4))
pixel.place(grid_v_in,grid_v_out, grid_m, grid_m_target)

loss, loss_standard, loss_position,loss_position_count, loss_velocity, loss_all, pio_all = scalar(), scalar(), scalar(), scalar(), scalar(), scalar(), scalar()
loss_field = scalar()
ti.root.place(loss, loss_standard,loss_position,loss_velocity, loss_all, pio_all,loss_position_count)
...
obj_center = vec()
ti.root.dense(ti.l,1).place(obj_center)
...
@ti.kernel
def compute_obj_center(obj:ti.template()):
    for p in range(obj.pio_n):
        obj_center[0] += obj.pio_x[max_step - 1, p] / obj.pio_n
        if obj.pio_x[max_step - 1, p][2] < 0.35:
            loss_position[None] += 0.35 - obj.pio_x[max_step - 1, p][2]
            loss_position_count[None] += 1
        elif obj.pio_x[max_step - 1, p][2] > 0.5:
            loss_position[None] += obj.pio_x[max_step - 1, p][2] - 0.45
            loss_position_count[None] += 1
@ti.kernel
def compute_loss(obj:ti.template(), hanger: ti.template(), target:ti.template(), v_input: ti.template(), minimum_index: ti.i32):
    # loss[None] +=  ((obj_center[0][0] - 0.5) ** 2) ** 0.5 + ((obj_center[0][1] - 0.5) ** 2) ** 0.5 + ((obj_center[0][2] - 0.4) ** 2) ** 0.5
    # loss[None] += loss_position[None] / loss_position_count[None]
    # loss[None] += ((obj.pio_x[max_step - 1, 0][1] - 0.55) ** 2) ** 0.5
    # loss_velocity[None] += ((obj.pio_x[max_step - 1, 0][1] - 0.55) ** 2) ** 0.5 
    
    # for p in range(obj.pio_n):
    #     dist = obj.pio_coefficient[p] * ti.math.length(obj.pio_x[minimum_index, p] - target[p])
    #     dist_vec = obj.pio_x[max_step-1, p] - target[p]
    #     dist_vec_abs = [(dist_vec[0]**2)**0.5,(dist_vec[1]**2)**0.5,(dist_vec[2]**2)**0.5]
    #     for i in ti.static(range(3)):
    #         loss[None] += obj.pio_coefficient[p][i] * dist_vec_abs[i] 
    #     loss_position[None] += ti.math.length(obj.pio_x[minimum_index, p] - target[p])
    ...
    for i,j,k in grid_m:
        dist = grid_m[i,j,k] - grid_m_target[i,j,k]
        loss[None] += dist ** 2
...
@ti.kernel
def update_pio_coe(obj:ti.template(), target:ti.template(),dist_nda: ti.types.ndarray(), iter_num: ti.i32,r_pio_coefficient: ti.template(),r_pio_dist:ti.template(),r_pio_dist_hat:ti.template()):
    for p in range(obj.pio_n):
        dist_vec = obj.pio_x[max_step-1, p] - target[p]
        dist_vec_abs = [(dist_vec[0]**2)**0.5,(dist_vec[1]**2)**0.5,(dist_vec[2]**2)**0.5]
        for i in ti.static(range(3)):
            dist_nda[1, p][i] = beta_loss * dist_nda[0, p][i] + (1 - beta_loss) * dist_vec_abs[i]
            dist_nda_hat = dist_nda[1, p][i] / (1 - beta_loss ** iter_num)
            dist_nda[0, p][i] -= dist_nda[0, p][i] 
            dist_nda[0, p][i]  +=  dist_nda[1, p] [i] 
            loss_all[None] += ti.pow(10,10*dist_nda_hat)
            r_pio_dist[iter_num,p][i] = dist_vec_abs[i]
            r_pio_dist_hat[iter_num,p][i] = dist_nda_hat


    for p in range(obj.pio_n):
        for i in ti.static(range(3)):
            dist_nda_hat = dist_nda[1, p][i] / (1 - beta_loss ** iter_num)
            obj.pio_coefficient[p][i] = ti.pow(10, 10*dist_nda_hat) / loss_all[None]
            r_pio_coefficient[iter_num,p][i] = obj.pio_coefficient[p][i]
            pio_all[None] += obj.pio_coefficient[p][i]
...
def record_data(iteration, r_loss, r_loss_standard, r_loss_position, r_loss_velocity,  r_v_input, r_grad,r_minimum_index,r_pio_coefficient,r_pio_dist,r_pio_dist_hat):
    r_loss[iteration] = loss[None]
    r_loss_standard[iteration] = loss_standard[None]
    r_loss_position[iteration] = loss_position[None] / loss_position_count[None]
    ...
    pio_coe_np = r_pio_coefficient.to_numpy()
    pio_dist_np = r_pio_dist.to_numpy()
    pio_dist_hat_np = r_pio_dist_hat.to_numpy()
    ...
    np.save('./data/pio_coe.npy',pio_coe_np)
    np.save('./data/pio_dist.npy',pio_dist_np)
    np.save('./data/pio_dist_hat.npy',pio_dist_hat_np)
...
def main():
    ...
    grid_m_target_np = np.load(f'./target_path/target_m_{target_num}.npy')
    grid_m_target.from_numpy(grid_m_target_np)
    ...
    r_pio_coefficient = ti.Vector.field(3, dtype = ti.f32, shape =  (500,obj.pio_n))
    r_pio_dist_hat = ti.Vector.field(3, dtype = ti.f32, shape =  (500,obj.pio_n))
    r_pio_dist = ti.Vector.field(3, dtype = ti.f32, shape =  (500,obj.pio_n))
    ...
    dist_nda = ti.ndarray(dtype=ti.math.vec3, shape=(2, obj.pio_n))
    ...
    while MPC_iter_num < max_MPC_iter:
        ...
        while iter_num <= max_iter:
            ...
            obj_center[0] = [0.0, 0.0, 0.0]
            loss[None] = 0.0
            loss_standard[None] = 0.0
            loss_position[None] = 0.0
            loss_position_count[None] = 1.0
            ...
            with ti.ad.Tape(loss=loss):
                ...
                compute_loss(obj, hanger, pio_target, v_input,max_step-1)

            #update_pio_coe(obj, pio_target, dist_nda, iter_num,r_pio_coefficient,r_pio_dist,r_pio_dist_hat)
            ...
            print('iter: ', iter_num,'loss:', loss[None], 'loss_pos:', loss_position[None])
            ...
            record_data(MPC_iter_num*(max_iter-1) + iter_num -1
                        , r_loss,r_loss_standard, r_loss_position, r_loss_velocity, r_v_input, r_grad, r_minimum_index,r_pio_coefficient,r_pio_dist,r_pio_dist_hat)
